...
import kira.ditto.data.pi.SharedPiHostToolCall
...
class SharedConversationMessagesTest {
    ...
    fun visibleReasoningModelsRouteNativeToolsIntoAReasoningTimeline() {
        val event = kira.ditto.data.pi.SharedPiToolEvent(
            id = "read-1",
            name = "read",
    ...
    @Test
    fun steeringQueuedTurnPromotesItForImmediateInjection() {
        val queued = mutableListOf(
            SharedPendingTurn(id = "wait", text = "later"),
            SharedPendingTurn(id = "now", text = "inject"),
        )
        assertTrue(queued.steerSharedPending("now"))
        assertEquals(SharedPendingTurnMode.Queue, queued[0].mode)
        assertEquals(SharedPendingTurnMode.Steer, queued[1].mode)
        assertFalse(queued.steerSharedPending("missing"))
        assertFalse(queued.steerSharedPending("now"))
    }
