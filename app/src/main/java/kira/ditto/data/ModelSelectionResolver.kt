...
fun resolveDefaultVectorModelKey(
    settings: AppSettings,
    providerConfigs: List<LlmProviderConfig>,
): String {
    val options = providerConfigs.availableModelOptions(includeDisabledModels = true)
    return normalizeSelectableModelKey(settings.defaultVectorModelKey, options)
}

fun resolveDefaultImageModelKey(
    settings: AppSettings,
    providerConfigs: List<LlmProviderConfig>,
): String {
    val options = providerConfigs.availableModelOptions(includeDisabledModels = true)
    return normalizeSelectableModelKey(settings.defaultImageModelKey, options)
}

fun resolveDefaultAsrModelKey(
    settings: AppSettings,
    providerConfigs: List<LlmProviderConfig>,
): String {
    val options = providerConfigs.availableModelOptions(includeDisabledModels = true)
    return normalizeAsrModelKey(settings.defaultAsrModelKey, options)
}
...
): AppSettings = resolveModelSettingsFromOptions(
...
fun resolveModelSettingsFromOptions(
    baseSettings: AppSettings,
    options: List<ProviderModelOption>,
    preferredModelKey: String,
    fallbackModelKey: String,
