...
fun mcpServersForAcpSession(
    servers: List<McpServerConfig>,
    selectedIds: Collection<String>,
): List<McpServerConfig> {
    val selected = selectedIds.toSet()
    return servers.filter { server ->
        server.isEnabled && server.id in selected
    }.distinctBy { it.id }
}

internal fun mergeShippedMcpServers(persisted: List<McpServerConfig>): List<McpServerConfig> {
    val stocks = listOf(
    ...
    val retired = setOf("aether-feishu")
    val withoutUpa = persisted.filterNot {
        kira.ditto.upa.isUpaMcpServerId(it.id) || it.id in retired
    }
    val stockIds = stocks.map(McpServerConfig::id).toSet()
    val merged = stocks.map { stock ->
        val existing = withoutUpa.firstOrNull { it.id == stock.id }
        stock.copy(
            isEnabled = existing?.isEnabled ?: true,
            createdAtMillis = existing?.createdAtMillis ?: stock.createdAtMillis,
            updatedAtMillis = existing?.updatedAtMillis ?: stock.updatedAtMillis,
        )
    }
    return (merged + withoutUpa.filterNot { it.id in stockIds })
        .sortedBy { it.displayName.lowercase() }
}

/**
 * The ACP `mcpServers` array for a session.
 *
 * `agentModeEnabled` and `browserEnabled` used to gate which shipped servers appeared here. They no
 * longer do, and the parameters are gone rather than ignored, so that a caller cannot believe it is
 * still controlling the tool list: the set of shipped servers is now a constant for the life of a
 * session, and availability is answered when a tool is called (see [McpToolAvailability]). Only the
 * user's own configured servers still vary, and changing those is an explicit configuration action
 * rather than something that happens mid-conversation.
 */
internal fun buildAcpMcpServerArray(
    servers: List<McpServerConfig>,
    selectedIds: Collection<String>,
    learningSessionId: String = "",
    secrets: McpSecretLookup = McpSecretLookup.None,
): JSONArray = JSONArray().apply {
    mcpServersForAcpSession(servers, selectedIds).forEach { server ->
        // A shipped server is declared below, by the shipped path, and only there.
        //
        // It can also appear in the user's own list, and the two paths do not produce the same
        // JSON: the shipped path attaches `x-aether-session-id`, the configured path does not. That
        // asymmetry made the *same* server hash differently depending on which path happened to add
        // it first, so selecting Gmail in the composer moved the fingerprint and recreated the ACP
        // session - the exact failure this whole change exists to remove.
        if (isShippedMcpServerId(server.id)) return@forEach
        if (missingMcpSecretSlots(listOf(server), listOf(server.id), secrets).isNotEmpty()) {
            return@forEach
        }
        applyMcpSecrets(server, secrets).toAcpMcpServer()?.let(::put)
    }
    // Shipped servers are declared unconditionally, whatever the user has toggled and whichever
    // secrets are present.
    //
    // The set of declared servers decides `acpMcpServersFingerprint`, which decides whether the ACP
    // session can be reused. Gating declaration on enablement therefore meant that switching an MCP
    // on mid-conversation did not just invalidate the prompt prefix - it recreated the session. So
    // availability moved to call time (`McpToolAvailability`, plus the `input_required` the secret-
    // backed hosts already returned), and the declaration became a constant.
    //
    // `already` guards remain: the same server can also arrive through the user-configured list.
    fun putShipped(name: String, server: JSONObject?) {
        if (server == null) return
        val duplicate = (0 until length()).any { index ->
            optJSONObject(index)?.optString("name") == name
        }
        if (!duplicate) put(server)
    }
    putShipped(GmailMcp.ServerName, GmailMcp.toAcpServer(learningSessionId))
    putShipped(SpotifyMcp.ServerName, SpotifyMcp.toAcpServer(learningSessionId))
    putShipped(AmapMcp.ServerName, AmapMcp.toAcpServer(learningSessionId))
    putShipped(GithubMcp.ServerName, GithubMcp.toAcpServer(learningSessionId))
    putShipped(HuggingFaceMcp.ServerName, HuggingFaceMcp.toAcpServer(learningSessionId))
    DeviceCatalogMcp.toAcpServer()?.let(::put)
    SessionMemoryMcp.toAcpServer(learningSessionId)?.let(::put)
    // Agent-mode and browser servers are declared for the same reason: toggling agent mode inside a
    // conversation used to change the tool list, which changed the fingerprint, which recreated the
    // session. The kimi profile `tools:` allowlist still keeps phone subagents away from browser
    // tools, and Gecko is still created lazily on the first webmcp call, so declaring costs nothing
    // until something is actually called.
    AgentDisplayMcp.toAcpServer(learningSessionId)?.let(::put)
    PhoneAppFlowMcp.toAcpServer(learningSessionId)?.let(::put)
    WebMcpMcp.toAcpServer(learningSessionId)?.let(::put)
}

internal fun acpMcpServersFingerprint(servers: JSONArray): String =
    buildList {
        for (index in 0 until servers.length()) {
            ...
            add(
                listOf(
                    ...
                    acpMcpHeadersFingerprint(item.optJSONArray("headers")),
                ).joinToString("\t"),
            )
        }
    }.sorted().joinToString("\n")

private fun acpMcpHeadersFingerprint(headers: JSONArray?): String {
...
sealed interface McpTransportConfig {
    ...
    data class UpaManifest(
        ...
        override val transportType: McpTransportType = McpTransportType.UpaManifest
    }
}
...
internal fun McpTransportConfig.toJson(): JSONObject = JSONObject().apply {
    ...
    when (this@toJson) {
        ...
        is McpTransportConfig.UpaManifest -> put("pluginId", pluginId)
    }
}
...
private fun parseMcpTransportConfig(json: JSONObject): McpTransportConfig? =
    when (McpTransportType.fromStorage(json.optString("type"))) {
        ...
        McpTransportType.UpaManifest -> McpTransportConfig.UpaManifest(
            pluginId = json.optString("pluginId").ifBlank { json.optString("plugin_id") },
        )
    }
...
internal fun McpServerConfig.toAcpMcpServer(): JSONObject? = when (val transport = transport) {
    is McpTransportConfig.StdIo -> JSONObject()
        .put("name", acpMcpServerName())
        .put("command", transport.command)
        .put("args", JSONArray(transport.arguments))
        .put(
            "env",
            JSONArray().apply {
                transport.environment.forEach { item ->
                    put(JSONObject().put("name", item.key).put("value", item.value))
                }
            },
        )

    is McpTransportConfig.StreamableHttp -> JSONObject()
        .put("name", acpMcpServerName())
        .put("type", "http")
        ...
        .put(
            "headers",
            JSONArray().apply {
                transport.headers.forEach { item ->
                    put(JSONObject().put("name", item.key).put("value", item.value))
                }
            },
        )

    is McpTransportConfig.UpaManifest -> JSONObject()
        .put("name", kimiMcpServerKey(transport.pluginId))
        .put("type", "http")
        .put("url", upaMcpHttpUrl(transport.pluginId))
        .put("headers", JSONArray())
}

private fun McpServerConfig.acpMcpServerName(): String =
    if (kira.ditto.upa.isUpaMcpServerId(id)) {
        kimiMcpServerKey(id.removePrefix(kira.ditto.upa.UpaMcpServerIdPrefix))
    } else {
        displayName
    }
...
private fun McpTransportConfig.quickActionSource(): String = when (this) {
    ...
    is McpTransportConfig.UpaManifest -> pluginId
}
