...
import kira.ditto.data.chatdb.DefaultWorkspaceId
...
class WorkspaceFileBridge(
    ...
    private val homeDirectory: String
        get() = alpineRuntime.homeDirectory

    /**
     * Where a space's files live, and therefore the agent's working directory.
     *
     * `/workspace` stays the container - it is the proot bind target - and each space is a
     * directory beneath it. Kimi Code CLI derives its own workspace key from this path
     * (`wd_<basename>_<sha256 prefix>`) and registers it on `session/new`, so handing it a
     * per-space path is all it takes for the CLI to file sessions separately too.
     *
     * This used to be a per-session choice driven by a settings toggle, which on a phone asked
     * the user to reason about project layouts they never chose. The space is the answer instead.
     */
    fun workspaceDirectory(workspaceId: String): String {
        val id = workspaceId.trim().ifEmpty { DefaultWorkspaceId }
        return "${alpineRuntime.workspaceRoot}/$id"
    }

    fun workspaceUploadsDirectory(workspaceId: String): String =
        "${workspaceDirectory(workspaceId)}/uploads"
    ...
    suspend fun importAttachmentToWorkspace(
        ...
        val destinationPath = "${workspaceUploadsDirectory(workspaceId)}/$safeFileName"
    ...
    suspend fun hasLegacySessionWorkspaces(): Boolean = withContext(Dispatchers.IO) {
        val legacyRootPath = "$homeDirectory/$PerSessionWorkspaceBaseDirectoryName"
        ...
        val raw = JSONObject(alpineRuntime.executeCommand(command))
    ...
    private suspend fun deletePathUnderRoot(
        ...
        val raw = JSONObject(alpineRuntime.executeCommand(command, awaitTimeoutMillis = 60_000L))
        if (!raw.optBoolean("ok")) {
            error(raw.optString("errmsg").ifBlank { raw.optString("stderr").ifBlank { "Couldn't delete $targetLabel." } })
        }
    }

    private suspend fun deleteLegacySessionWorkspace(sessionId: String): Result<Unit> =
        deletePathUnderRoot(
            path = legacySessionWorkspaceDirectory(sessionId),
            rootPath = legacyWorkspacesRootDirectory(),
            targetLabel = "legacy session workspace",
            skipUnsafePath = true,
        )
    ...
    suspend fun deleteSessionsRuntimeData(
        ...
        runCatching {
            ...
            // A conversation no longer owns a directory - its space does - so only the files it
            // put there are removed here. Deleting the space is a separate, explicit act.
            uniqueSessionIds.forEach { sessionId ->
                deleteLegacySessionWorkspace(sessionId = sessionId).onFailure { error ->
                    failures += "legacy workspace $sessionId: ${error.message.orEmpty()}"
                }
            }
     ...
     *
     * The CLI keeps its own record of this directory in `workspaces.json`; it is left alone
     * deliberately. That file is the CLI's, it already carries a `deleted_workspace_ids` list for
     * exactly this, and a host that edits it is competing with the process that owns it.
     */
    suspend fun deleteWorkspaceDirectory(workspaceId: String): Result<Unit> = deletePathUnderRoot(
        path = workspaceDirectory(workspaceId),
        rootPath = alpineRuntime.workspaceRoot,
        targetLabel = "workspace $workspaceId",
        skipUnsafePath = true,
    )

    /**
     * Delete space directories with no space behind them any more, and the leftovers from before
     * spaces existed.
     *
     * This is the sweep that catches what the other two paths cannot: a delete that failed while
     * the guest was down, a crash between the two writes, and - the reason it exists - the files
     * that accumulated in the old shared root back when every conversation ran there. One of those
     * leftovers was read by a brand-new conversation as if it were the task at hand.
     *
     * `.aether` is kept: it is host metadata, not user content.
     */
    suspend fun deleteOrphanWorkspaceDirectories(
        knownWorkspaceIds: Collection<String>,
    ): Result<Unit> = withContext(Dispatchers.IO) {
        runCatching {
            val root = alpineRuntime.workspaceRoot
            val keep = knownWorkspaceIds
                .map(String::trim)
                .filter(String::isNotEmpty)
                .toSet() + ".aether"
            val listCommand = buildString {
                appendCommonShellPreamble(this)
                appendLine("root=\"\$(decode_b64 '${encodeBase64(root)}')\"")
                appendLine("if [ -d \"\$root\" ]; then")
                appendLine("  ls -A \"\$root\" 2>/dev/null | while IFS= read -r entry; do")
                appendLine("    emit_kv entry \"\$entry\"")
                appendLine("  done")
                appendLine("fi")
            }
            val raw = JSONObject(alpineRuntime.executeCommand(listCommand))
            if (!raw.optBoolean("ok")) return@runCatching
            val entries = raw.optString("stdout")
                .lineSequence()
                .mapNotNull { line -> line.substringAfter("entry=", "").trim().takeIf(String::isNotEmpty) }
                .distinct()
                .toList()

            val failures = mutableListOf<String>()
            entries.filterNot { it in keep }.forEach { orphan ->
                deletePathUnderRoot(
                    path = "$root/$orphan",
                    rootPath = root,
                    targetLabel = "orphan workspace $orphan",
                    skipUnsafePath = true,
                ).onFailure { error -> failures += "$orphan: ${error.message.orEmpty()}" }
            }
            if (failures.isNotEmpty()) {
                error("Failed to clean up ${failures.size} orphan workspace(s): ${failures.joinToString(" | ")}")
            }
        }
    }

    private fun legacySessionWorkspaceDirectory(sessionId: String): String =
        "$homeDirectory/$PerSessionWorkspaceBaseDirectoryName/$sessionId"

    private fun legacyWorkspacesRootDirectory(): String =
        "$homeDirectory/$PerSessionWorkspaceBaseDirectoryName"

    private fun sharedWorkspaceUploadsDirectory(): String =
        "${alpineRuntime.workspaceRoot}/uploads"
    ...
    fun resolveTermuxPath(
        ...
        return when {
            ...
            trimmedPath == "~" -> homeDirectory
            trimmedPath.startsWith("~/") -> homeDirectory + trimmedPath.removePrefix("~")
    ...
    private suspend fun readWorkspaceFileChunk(
        ...
        val rawResult = JSONObject(alpineRuntime.executeCommand(command))
        ensureBashSuccess(
            rawResult = rawResult,
