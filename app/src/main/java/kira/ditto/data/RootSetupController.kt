...
class RootSetupController(
    ...
    suspend fun configureLocalAccess(): RootSetupState = withContext(Dispatchers.IO) {
        ...
        val commandResult = runProcess(
            command = listOf(suPath, "-c", "id && printf '\\n$RootSetupReadyMarker\\n'"),
            timeoutMillis = 12_000L,
        )
        val combined = commandResult.combinedOutput()
        if (commandResult.timedOut || commandResult.exitCode != 0 || !commandResult.stdout.contains(RootSetupReadyMarker)) {
            val detail = combined.ifBlank {
                if (commandResult.timedOut) {
                    "Root request timed out. Grant su to Aether, then try again."
                } else {
                    commandResult.launchError.ifBlank { "Root probe failed." }
                }
            }
            diagnosticLogger.event(
                category = "root_setup",
        ...
        RootSetupState(
            ...
            detail = "Root is available for Agent Mode.",
            rootAvailable = true,
            suPath = suPath,
