...
class SettingsRepository(
    ...
    val settings: Flow<AppSettings> = context.dataStore.data.map { preferences ->
        ...
        AppSettings(
            ...
            defaultVectorModelKey = preferences[DEFAULT_VECTOR_MODEL_KEY].orEmpty(),
            defaultImageModelKey = preferences[DEFAULT_IMAGE_MODEL_KEY].orEmpty(),
            defaultAsrModelKey = preferences[DEFAULT_ASR_MODEL_KEY].orEmpty(),
            defaultTtsModelKey = preferences[DEFAULT_TTS_MODEL_KEY].orEmpty(),
            ttsEnabled = preferences[TTS_ENABLED] ?: false,
            ttsVoiceId = preferences[TTS_VOICE_ID].orEmpty(),
            ...
            upaRenderUi = preferences[UPA_RENDER_UI] ?: true,
            upaDisabledUiPlugins = parseStringSet(preferences[UPA_DISABLED_UI_PLUGINS].orEmpty()),
            upaRevokedPermissions = parseStringListMap(preferences[UPA_REVOKED_PERMISSIONS].orEmpty()),
            upaMcpBindings = parseStringListMap(preferences[UPA_MCP_BINDINGS].orEmpty()),
            remoteMachines = parseRemoteMachines(preferences[REMOTE_MACHINES].orEmpty()),
            browserPreferences = parseBrowserPreferences(preferences[BROWSER_PREFERENCES].orEmpty()),
        ).also { UpaPluginHostState.publish(it) }
    }
    ...
    suspend fun replaceImportedSettings(
        ...
        context.dataStore.edit {
            ...
            it[DEFAULT_VECTOR_MODEL_KEY] = settings.defaultVectorModelKey
            it[DEFAULT_IMAGE_MODEL_KEY] = settings.defaultImageModelKey
            it[DEFAULT_ASR_MODEL_KEY] = settings.defaultAsrModelKey
            it[DEFAULT_TTS_MODEL_KEY] = settings.defaultTtsModelKey
            it[TTS_ENABLED] = settings.ttsEnabled
            it[TTS_VOICE_ID] = settings.ttsVoiceId
            ...
            it[UPA_RENDER_UI] = settings.upaRenderUi
            it[UPA_DISABLED_UI_PLUGINS] = serializeStringSet(settings.upaDisabledUiPlugins)
            it[UPA_REVOKED_PERMISSIONS] = serializeStringListMap(settings.upaRevokedPermissions)
            it[UPA_MCP_BINDINGS] = serializeStringListMap(settings.upaMcpBindings)
            it[REMOTE_MACHINES] = serializeRemoteMachines(settings.remoteMachines)
            it[BROWSER_PREFERENCES] = serializeBrowserPreferences(settings.browserPreferences)
    ...
    suspend fun updateUpaRenderUi(enabled: Boolean) {
        context.dataStore.edit { it[UPA_RENDER_UI] = enabled }
    }

    suspend fun updateUpaPluginRenderUi(pluginId: String, enabled: Boolean) {
        val id = pluginId.trim()
        if (id.isBlank()) return
        context.dataStore.edit { prefs ->
            val disabled = parseStringSet(prefs[UPA_DISABLED_UI_PLUGINS].orEmpty()).toMutableSet()
            if (enabled) disabled.remove(id) else disabled.add(id)
            prefs[UPA_DISABLED_UI_PLUGINS] = serializeStringSet(disabled)
            if (enabled) prefs[UPA_RENDER_UI] = true
        }
    }

    suspend fun updateUpaPluginPermission(pluginId: String, wire: String, granted: Boolean) {
        val id = pluginId.trim()
        val permission = wire.trim()
        if (id.isBlank() || permission.isBlank()) return
        context.dataStore.edit { prefs ->
            val map = parseStringListMap(prefs[UPA_REVOKED_PERMISSIONS].orEmpty()).toMutableMap()
            val revoked = map[id].orEmpty().toMutableSet()
            if (granted) revoked.remove(permission) else revoked.add(permission)
            if (revoked.isEmpty()) map.remove(id) else map[id] = revoked.toList()
            prefs[UPA_REVOKED_PERMISSIONS] = serializeStringListMap(map)
        }
    }

    suspend fun updateUpaPluginMcpBinding(pluginId: String, serverId: String, selected: Boolean) {
        val id = pluginId.trim()
        val server = serverId.trim()
        if (id.isBlank() || server.isBlank()) return
        context.dataStore.edit { prefs ->
            val map = parseStringListMap(prefs[UPA_MCP_BINDINGS].orEmpty()).toMutableMap()
            val bound = map[id].orEmpty().toMutableList()
            if (selected && server !in bound) bound.add(server)
            if (!selected) bound.removeAll { it == server }
            if (bound.isEmpty()) map.remove(id) else map[id] = bound
            prefs[UPA_MCP_BINDINGS] = serializeStringListMap(map)
        }
    }

    suspend fun enableAgentModeShizuku() {
        context.dataStore.edit {
            it[AGENT_MODE_AUTHORIZATION_ENABLED] = true
            it[AGENT_MODE_AUTHORIZATION_METHOD] =
                AgentModeAuthorizationMethod.Shizuku.storageValue
        }
    }
    ...
    private companion object {
        ...
        val DEFAULT_VECTOR_MODEL_KEY = stringPreferencesKey("default_vector_model_key")
        val DEFAULT_IMAGE_MODEL_KEY = stringPreferencesKey("default_image_model_key")
        val DEFAULT_ASR_MODEL_KEY = stringPreferencesKey("default_asr_model_key")
        val DEFAULT_TTS_MODEL_KEY = stringPreferencesKey("default_tts_model_key")
        ...
        val UPA_RENDER_UI = booleanPreferencesKey("upa_render_ui")
        val UPA_DISABLED_UI_PLUGINS = stringPreferencesKey("upa_disabled_ui_plugins")
        val UPA_REVOKED_PERMISSIONS = stringPreferencesKey("upa_revoked_permissions")
        val UPA_MCP_BINDINGS = stringPreferencesKey("upa_mcp_bindings")
        val REMOTE_MACHINES = stringPreferencesKey("remote_machines")
        val BROWSER_PREFERENCES = stringPreferencesKey("browser_preferences")
    }
}
...
private fun parseStringSet(rawValue: String): Set<String> = parseStringArray(rawValue).toSet()

private fun serializeStringSet(values: Set<String>): String = serializeStringArray(values.toList())

private fun parseStringListMap(rawValue: String): Map<String, List<String>> {
...
private fun serializeStringListMap(values: Map<String, List<String>>): String {
    val obj = JSONObject()
    values.forEach { (key, list) ->
        obj.put(key, JSONArray(list))
    }
    return obj.toString()
}
...
private fun parseRemoteMachines(rawValue: String): List<RemoteMachine> {
    if (rawValue.isBlank()) return emptyList()
    return runCatching {
        val array = JSONArray(rawValue)
        buildList {
            for (index in 0 until array.length()) {
                ...
                add(
                    RemoteMachine(
                        id = id,
                        name = item.optString("name").trim().ifBlank { "Kimi" },
                        baseUrl = baseUrl,
                        token = item.optString("token"),
                        cwd = item.optString("cwd"),
                        lastSeenAtMillis = item.optLong("lastSeenAtMillis"),
                    ),
                )
            }
        }
...
private fun serializeRemoteMachines(machines: List<RemoteMachine>): String =
    JSONArray().apply {
        machines.forEach { machine ->
            put(
                JSONObject().apply {
                    ...
                    put("baseUrl", machine.baseUrl)
                    put("token", machine.token)
                    put("cwd", machine.cwd)
                    put("lastSeenAtMillis", machine.lastSeenAtMillis)
                },
            )
        }
    }.toString()
...
private fun serializeModelCatalogCache(cache: Map<String, ModelCatalogInfo>): String = JSONArray().apply {
    cache.forEach { (key, info) ->
        put(JSONObject().apply {
            ...
            info.contextWindow?.let { put("contextWindow", it) }
        })
    }
}.toString()
    ...
    buildMap {
        for (index in 0 until array.length()) {
            ...
            put(key, ModelCatalogInfo(
                ...
                contextWindow = item.optLong("contextWindow").takeIf { it > 0L },
            ))
        }
    }
}.getOrDefault(emptyMap())
