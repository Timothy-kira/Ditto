...
class ProviderModelCatalogClientTest {
    ...
    @Test
    fun stepFunUsesOfficialModelDiscoveryEndpoint() {
        assertEquals(
            "https://api.stepfun.com/v1/models",
            providerModelsEndpoint("https://api.stepfun.com"),
        )
        assertEquals(
            "https://api.stepfun.com/v1/models",
            providerModelsEndpoint("https://api.stepfun.com/step_plan"),
        )
    }
    ...
    @Test
    fun catalogMissDoesNotInventThinkingLevels() {
        val config = LlmProviderConfig(
            providerId = "stepfun",
            name = "StepFun",
            ...
            cachedModels = listOf("step-3.7-flash"),
            enabledModelIds = listOf("step-3.7-flash"),
        )
        val option = listOf(config).availableModelOptions().single()
        val result = publicCatalogThinkingResult(JSONObject("""{"providers":{}}"""), listOf(option))
        val key = thinkingCatalogKey("stepfun", "step-3.7-flash")

        assertEquals(false, key in result.levelsByProviderModel)
        assertEquals(emptySet<String>(), result.reasoningModels)
    }

    @Test
    fun probeReadsAllowedEffortsFromProviderError() = runBlocking {
        val server = MockWebServer()
        server.enqueue(
            MockResponse()
                .setResponseCode(400)
                .setBody("""{"error":{"message":"Invalid reasoning_effort. Supported values are: \"low\", \"medium\", \"high\""}}"""),
        )
        server.start()
        try {
            val option = listOf(
                LlmProviderConfig(
                    ...
                    apiKey = "test-key",
                    baseUrl = server.url("/v1").toString(),
                    modelId = "mystery-reasoner",
                    cachedModels = listOf("mystery-reasoner"),
                    enabledModelIds = listOf("mystery-reasoner"),
                ),
            ).availableModelOptions().single()

            val result = ProviderModelCatalogClient.probeProviderThinkingLevels(option)

            assertEquals(listOf("low", "medium", "high"), result.levels)
            assertEquals(true, result.resolved)
            val request = server.takeRequest()
            assertEquals("/v1/chat/completions", request.path)
            assertEquals(true, request.body.readUtf8().contains(ThinkingEffortProbeSentinel))
        } finally {
            server.shutdown()
        }
    }
    ...
    @Test
    fun probeTreatsUnknownParameterAsNoEffortSelector() = runBlocking {
        val server = MockWebServer()
        server.enqueue(
            MockResponse()
                .setResponseCode(400)
                .setBody("""{"error":{"message":"Unknown parameter: reasoning_effort"}}"""),
        )
        server.enqueue(
            MockResponse()
                .setResponseCode(400)
                .setBody("""{"error":{"message":"Unknown field: output_config"}}"""),
        )
        server.start()
        try {
            val option = listOf(
                LlmProviderConfig(
                    ...
                    baseUrl = server.url("/v1").toString(),
                    modelId = "plain-chat",
                    cachedModels = listOf("plain-chat"),
                    enabledModelIds = listOf("plain-chat"),
                ),
            ).availableModelOptions().single()

            val result = ProviderModelCatalogClient.probeProviderThinkingLevels(option)

            assertEquals(emptyList<String>(), result.levels)
            assertEquals(true, result.resolved)
            assertEquals(false, result.reasoningSupported)
