...
interface ChatHistoryDao {
    ...
    @Query("UPDATE chat_sessions SET workspaceId = :toWorkspaceId WHERE workspaceId = :fromWorkspaceId")
    suspend fun moveSessionsToWorkspace(fromWorkspaceId: String, toWorkspaceId: String)
    ...
    @Query("SELECT * FROM chat_agent_sessions")
    fun observeAgentSessions(): Flow<List<ChatAgentSessionEntity>>
    ...
    @Query("""
        SELECT sessionId, id, position, author, text, createdAtMillis, responseGroupId, displayKind, messageSchemaVersion, length(messageJson) AS messageJsonLength, isIncomplete, payloadHash
        FROM chat_messages
    ...
    /**
     * Newest window of a conversation. Returned newest-first so SQLite can walk the
     * `(sessionId, position)` index backwards and stop at [limit]; callers reverse it.
     * Positions are temporarily negated while a response checkpoint is parked, so the
     * window deliberately ignores negative positions.
     */
    @Query("""
        SELECT sessionId, id, position, author, text, createdAtMillis, responseGroupId, displayKind, messageSchemaVersion, length(messageJson) AS messageJsonLength, isIncomplete, payloadHash
        FROM chat_messages
        ...
        LIMIT :limit
    """)
    fun observeLatestMessageSummaries(
        sessionId: String,
        limit: Int,
    ): Flow<List<ChatMessageSummaryEntity>>
    ...
    @Query("""
        ...
        LIMIT :limit
    """)
    suspend fun getLatestMessageSummaries(
        sessionId: String,
        limit: Int,
    ): List<ChatMessageSummaryEntity>

    /** Keyset page: the [limit] messages immediately preceding [beforePosition]. */
    @Query("""
        SELECT sessionId, id, position, author, text, createdAtMillis, responseGroupId, displayKind, messageSchemaVersion, length(messageJson) AS messageJsonLength, isIncomplete, payloadHash
        FROM chat_messages
        WHERE sessionId = :sessionId AND position >= 0 AND position < :beforePosition
        ORDER BY position DESC
        LIMIT :limit
    """)
    suspend fun getMessageSummariesBefore(
        sessionId: String,
        beforePosition: Int,
        limit: Int,
    ): List<ChatMessageSummaryEntity>
    ...
    @Query("""
        SELECT length(payloadJson)
        FROM chat_message_payloads
        WHERE sessionId = :sessionId AND messageId = :messageId
    """)
    suspend fun getMessagePayloadLength(sessionId: String, messageId: String): Int?

    @Query("""
        SELECT substr(payloadJson, :start, :length)
        FROM chat_message_payloads
        WHERE sessionId = :sessionId AND messageId = :messageId
    """)
    suspend fun getMessagePayloadChunk(
        sessionId: String,
        messageId: String,
        start: Int,
        length: Int,
    ...
    @Upsert
    suspend fun upsertMessagePayloads(payloads: List<ChatMessagePayloadEntity>)

    @Query("DELETE FROM chat_message_payloads WHERE sessionId = :sessionId AND messageId IN (:messageIds)")
    suspend fun deleteMessagePayloads(sessionId: String, messageIds: List<String>)

    /** Stored fingerprints for a session, used to write only the rows that actually changed. */
    @Query("""
        SELECT id, position, contentHash, payloadHash
        FROM chat_messages
        WHERE sessionId = :sessionId AND position >= :fromPosition
    """)
    suspend fun getMessageDigestsFromPosition(
        sessionId: String,
        fromPosition: Int,
    ): List<ChatMessageDigestEntity>

    @Query("DELETE FROM chat_messages WHERE sessionId = :sessionId AND id IN (:messageIds)")
    suspend fun deleteMessagesByIds(sessionId: String, messageIds: List<String>)

    @Query("DELETE FROM chat_workspace_file_refs WHERE sessionId = :sessionId AND messageId IN (:messageIds)")
    suspend fun deleteWorkspaceFileRefsForMessages(sessionId: String, messageIds: List<String>)
    ...
    @Query("SELECT * FROM memory_original_pointers WHERE hash = :hash")
    suspend fun getMemoryOriginalPointer(hash: String): MemoryOriginalPointerEntity?

    @Upsert
    suspend fun upsertMemoryOriginalPointer(pointer: MemoryOriginalPointerEntity)

    @Query("SELECT * FROM chat_messages WHERE sessionId = :sessionId AND id IN (:messageIds)")
    suspend fun getMessagesByIds(sessionId: String, messageIds: List<String>): List<ChatMessageEntity>
}
