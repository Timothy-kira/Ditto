...
data class PiProviderDefinition(
    ...
    val defaultMessageApi: String = "builtin",
)
...
object PiProviderCatalog {
    val providers: List<PiProviderDefinition> = listOf(
        ...
        builtin(
            ...
            "step-3.7-flash",
            category = "China",
            defaultMessageApi = "anthropic-messages",
        ),
        ...
        builtin(
            ...
            "dots3-note-prev",
            category = "China",
            defaultMessageApi = "openai-completions",
        ),
        builtin(
            "miromind",
            "MiroMind",
    ...
    fun messageApiFor(providerId: String, modelId: String): String {
        val definition = resolve(providerId)
        return if (definition.id == "stepfun") {
            stepfunMessageApi(modelId)
        } else {
            definition.defaultMessageApi
        }
    }
}

fun stepfunIsFlashReasoningModel(modelId: String): Boolean {
    val id = modelId.trim().lowercase()
    return id.startsWith("step-3.7") || id.startsWith("step-3.5")
}

fun stepfunUsesStepPlan(modelId: String): Boolean {
    val id = modelId.trim().lowercase()
    return id == "step-explore" ||
        id.startsWith("step-router") ||
        stepfunIsFlashReasoningModel(id)
}

fun stepfunMessageApi(modelId: String): String {
    val id = modelId.trim().lowercase()
    return if (id == "step-explore" || id.startsWith("step-router")) {
        "anthropic-messages"
    } else {
        "openai-completions"
    }
...
fun normalizePiProviderBaseUrl(
    providerId: String,
    modelId: String,
    baseUrl: String,
): String {
    ...
    val isAiHost = normalized.startsWith("https://api.stepfun.ai", ignoreCase = true)
    val root = if (isAiHost) "https://api.stepfun.ai" else "https://api.stepfun.com"
    return if (!stepfunUsesStepPlan(modelId)) {
        "$root/v1"
    } else if (stepfunMessageApi(modelId) == "anthropic-messages") {
        "$root/step_plan"
    } else {
        "$root/step_plan/v1"
    }
...
private fun builtin(
    ...
    category: String,
    defaultMessageApi: String = "builtin",
    ...
    category = category,
    defaultMessageApi = defaultMessageApi,
)
...
private fun builtInProviderIdForHost(host: String): String? = when (host) {
    ...
    "note3-prev-api.askdiandian.com" -> "dots"
    "api.miromind.ai" -> "miromind"
    else -> null
}
